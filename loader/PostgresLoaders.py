...
class PostgresLoader(BaseLoader):
    ...
    def __init__(self, name=None, **parameters):
        ...
        self.run_tests = self.get_arg('run_tests')
    ...
    def init_args(self):
        ...
        self.argparser.add_argument('--run_tests', type=str)
    ...
    def generate_copy_query(self, columns, dt=None):
        ...
        # check if updated_at exists in table
        # if exists, filter data by partitioning_field + updated_at columns
        # TODO: expand to multiple fields filter
        ...
        # if dt is given then filter data, else upload full table
        if dt:
            return f'''COPY(
                SELECT
    ...
    def create_table_saa(self):
        ...
        TODO: If table exists, but schema is different throw assert and return False
        '''
        ...
        table_id = f'{self.env.project_id}.saa.{self.target_table}'
    ...
    def data_to_saa(self, columns, saa_schema, dt=None):
        '''
        Loads data from csv file to bq_saa schema
        :param dt: upload data. given iterativly
        :param columns: columns of table. needs to generate copy query
        :param saa_schema: schema of bq table (as bq class). needs for bq job
        '''
        ...
        dataset_ref = self.bq.client.dataset('saa')
    ...
    def create_table_snp(self, table_schema):
        '''
        Check if table exists. If not -> creates it.
        TODO: pass the fields nedd to be parsed
        https://cloud.google.com/bigquery/docs/nested-repeated
        example of nested fields creating
        '''
    ...
    def data_to_snp(self, columns):
        '''
        Delete duplicated data from snp and write fresh from current launch
        '''
    ...
    def check_completeness(self):
        '''
        Check that amount of data in postrges and bq is the same
        '''
        pass

    def run(self):
        # создадим или проверим наличие нужных таблиц
        # TODO: на этапе create_table_saa проверять поменялась ли схема. если да, то сносить таблицы в saa и snp и перезагружать всю историю
        ...
        if self.date_trunc:
            ...
            for dt in tr_dates:
                ...
                self.data_to_saa(columns, saa_schema, dt)
                self.data_to_snp(columns)
        # если date_trunc пустой то просто грузим всю таблицу. для этого просто не передаем параметр dt
        else:
            self.data_to_saa(columns, saa_schema)
            self.data_to_snp(columns)
